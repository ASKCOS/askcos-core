...
class TestMolGraph(unittest.TestCase):
    def test_01_smiles2graph_list(self):
        ...
        with open(os.path.join(os.path.dirname(__file__), 'test_data/mg_smiles2graph_list.pkl'), 'rb') as t:
...
class TestMolGraphRich(unittest.TestCase):
    def test_01_smiles2graph_list(self):
        ...
        with open(os.path.join(os.path.dirname(__file__), 'test_data/mgr_smiles2graph_list.pkl'), 'rb') as t:
            expected = pickle.load(t, encoding='iso-8859-1')
        for e, r in zip(expected, result):
            self.assertTrue((e == r).all())
